...
async def create_result(result_in: JobResultCreate, db: AsyncSession = Depends(get_db)):
...
async def list_results(skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db)):
    q = select(JobResult).offset(skip).limit(limit)
...
async def update_result(result_id: uuid.UUID, result_in: JobResultUpdate, db: AsyncSession = Depends(get_db)):
    item = await db.get(JobResult, result_id)
    if not item:
        raise HTTPException(status_code=404, detail="Result not found")
